        ...
        <div class="add-node bg_grad_color" id="drop-area">
            <div class="wrapper wrap-node">
                <div id="drawflow" ondrop="drop(event)" ondragover="allowDrop(event)" class="parent-drawflow " tabindex="0">
                    <div class="btn-clear edit" id="node_edit" style="display:none;">Edit</div>
                    <?php
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="modal-header px-0 admin_only">
                    <h5 class="modal-title" id="reportModalLabel">report <span id="reportCount">(0)</span></h5>
                </div>
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            `);
                                ...
                                $commentList.append(`<p class="empty-report" >No reports yet.!</p>`);
                            }
                        } else {
                            console.log("Error:", response.data);
                        }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            // const currentCount = parseInt($("#commentCount").text().replace(/[()]/g, '')) - 1;
                            $popup.find("#reportCount").text(`(${reportCount})`);
                            if (reportCount === 0) {
                                $("#reportList").find(".empty-report").show(); // Show when no comments
                            } else {
                                $("#reportList").find(".empty-report").hide(); // Hide when comments exist
                            }
                        } else {
                            alert(response.data.message);
                        }
    ...
    jQuery(document).ready(function($) {
        ...
        $('#node_edit').on('click', function() {
            ...
            var form = $('<form>', {
                'method': 'POST',
                'action': '/node-update/',
                'style': 'display: none;' // Make the form visible for debugging
            });
